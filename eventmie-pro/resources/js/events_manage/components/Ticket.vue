                ...
                <div class="modal-content">
                    ...
                    <form ref="form" @submit.prevent="validateForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label for="coupon_content">{{ trans('em.coupon') }}</label>
                                <input type="text" class="form-control lgxname"  name="coupon_content" v-model="coupon_content">
                                <span v-show="errors.has('coupon')" class="help text-danger">{{ errors.first('coupon_content') }}</span>
                            </div>

                            <div class="form-group">
                                <label for="coupon_value">{{ trans('em.coupon_percent') }} %</label>
                                <input type="text" class="form-control lgxname"  name="coupon_value" v-model="coupon_value">
                                <span v-show="errors.has('coupon_calue')" class="help text-danger">{{ errors.first('coupon_value') }}</span>
                            </div>
...
export default {
    ...
    methods: {
        ...
        editTicket() {
            ...
            this.coupon_content = this.edit_ticket.coupon_content;
            this.coupon_value = this.edit_ticket.coupon_value;
        ...
        setTaxesOptions(){
            ...
            if(Object.keys(this.taxes).length > 0)
            {
                this.taxes.forEach(function(v, key) {
                    this.taxes_options.push({value : v.id, text : v.title });
                }.bind(this));
            }
        },
        // show selected taxes
        setSelcetedTaxes(){
            ...
            if(Object.keys(this.edit_ticket.taxes).length > 0)
            {
                ...
                this.edit_ticket.taxes.forEach(function (v, key) {
                    this.tmp_taxes_ids.push({value : v.id, text : v.title });
                }.bind(this));
