            ...
            <div class="lgx-events">
                <a :href="eventSlug(event.slug)" >
                    ...
                    <div class="lgx-event__tag"
                         v-if="!event.repetitive && moment().format('YYYY-MM-DD') == convert_date_to_local(event.start_date, 'YYYY-MM-DD')"
                    >
                        <span class="compForEvent__span">{{ trans('em.today') }}</span>
                        <span class="compForEvent__span">{{ trans('em.event') }}</span>
                    </div>
                    ...
                    <div class="lgx-event__image">
                        <img :src="'/storage/'+event.thumbnail" alt="" class="event__image">
                    </div>
                    ...
                    <div class="lgx-event__info">
                        ...
                        <div class="meta-wrapper">
                            ...
                            <span class="compForEvent__span"> {{changeDateFormat(convert_date_to_local(event.start_date), "YYYY-MM-DD")}}</span>

                            <span class="compForEvent__span" v-if="event.start_date != event.end_date">{{ changeDateFormat(convert_date_to_local(event.end_date), "YYYY-MM-DD")}} </span>
                            <span class="compForEvent__span" v-else>{{ changeTimeFormat(event.start_time)}} - {{ changeTimeFormat(event.end_time)}} </span>

                            <span class="compForEvent__span">{{event.city}}</span>
                        </div>
                    ...
                    <div class="lgx-event__footer" style="display: none;">
                        <div
                            v-for="(ticket, index1) in event.tickets"
                            :key="index1"
                            v-if="index1 <= 1"
                    ...
                    <div class="lgx-event__category" style="padding-top: 10%;">
