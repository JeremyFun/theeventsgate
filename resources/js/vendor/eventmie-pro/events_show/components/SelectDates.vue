...
<template>
    <div class="col-xs-12">
        <div class="lgx-tab" v-if="event.repetitive > 0">
                                        ...
                                        >
                                            <div class="schedule-info" 
                                                ...
                                                <h4 class="time event-outofstock mr-3" v-if="
                                                    !(moment(dateToFullDate(selected_date, schedules[index].from_date),'dddd LL').add(1, 'day').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')) && positiveInteger(available_dates[index][index1])"
                                                >
            ...
            <div class="tab-content lgx-tab-content" v-if="event.merge_schedule > 0 && event.repetitive_type == 3">
                                    ...
                                    <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                                        ...
                                        >
                                            <div class="schedule-info pl-0"> 
                                                ...
                                                <!-- filling fast -->
                                                <h4 class="time event-outofstock mr-3" v-if="
                                                    !(moment(dateToFullDate(selected_dates[index][0],schedules[index].from_date),'dddd LL').add(1, 'day').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')) &&  positiveInteger(available_dates[index][0])"
                                                >
                                                    <span>{{ trans('em.filling_fast') }}</span>
                                                </h4>
            ...
            <div class="tab-content lgx-tab-content" v-if="event.merge_schedule > 0 && event.repetitive_type == 2">
                <div v-for="(item, index) in calculate_months" :key="index" class="tab-pane" :class="{'active' : index == tab_active_index}" >
                    <div class="panel-group" role="tablist" aria-multiselectable="true">
                        <div class="panel panel-default lgx-panel">
                            <div class="panel-heading" role="tab">
                                    ...
                                    <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                                        ...
                                        >
                                            <div class="schedule-info pl-0"> 
                                                ...
                                                <!-- filling fast -->
                                                <h4 class="time event-outofstock mr-3" v-if="
                                                    !(moment(getWeekFirstDate(item2, selected_dates[index], schedules[index].from_date)).add(1, 'day').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')) && positiveInteger(checkSeatAvailability(moment(getWeekFirstDate(item2, selected_dates[index], schedules[index].from_date)).format('YYYY-MM-DD')))"
                                                >
                                                    <span>{{ trans('em.filling_fast') }}</span>
                                                </h4>
                            ...
                            <div class="panel-heading" role="tab">
                                <div class="panel-title">
                                    <a role="button" data-toggle="collapse" aria-expanded="true" aria-controls="collapseOne">
                                        <div class="lgx-single-schedule"
                                            :class="{
                                                'expired-event': moment(event.start_date,'YYYY-MM-DD' ).add(1, 'day').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD'),
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <h4 class="time">{{ changeTimeFormat(event.start_time) }} - {{ changeTimeFormat(event.end_time) }}</h4>

                                                <!-- expired -->
                                                <h4 class="time event-ended" v-if="moment(event.start_date, 'YYYY-MM-DD').add(1, 'day').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')"
                                                >
                                                    <span>{{ trans('em.ended') }}</span>
                                                </h4>
                                                ...
                                                <!-- filling fast -->
                                                <h4 class="time event-outofstock mr-3" v-if="
                                                    !(moment(event.start_date, 'YYYY-MM-DD').add(1, 'day').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')) && positiveInteger(checkSeatAvailability(moment(event.start_date,'YYYY-MM-DD' ).format('YYYY-MM-DD')))"
                                                >
                                                    <span>{{ trans('em.filling_fast') }}</span>
                                                </h4>
    ...
    methods: {
        ...
        selectDates(booking_date, booking_end_date, start_time, end_time)
        {
            this.add({
                ...
                booked_date_server  : (moment(booking_date,'dddd LL').isValid())  ? moment(booking_date, "dddd LL").format('YYYY-MM-DD') : null,
        ...
        getEventSchedule()
        {
            ...
            .then(res => {
                ...
                this.calculate_months = this.countMonth(this.local_start_date, this.local_end_date);
        ...
        generateMonthly() {
            ...
            this.all_dates.forEach(function(ad_value, ad_key) {
                ...
                var schedules_dates = [];
                $this.schedules[ad_key].repetitive_dates.split(',').forEach(function(v, k) {
                    schedules_dates[k] = v;
                });
                ...
                ad_value.forEach(function(date, key) {
                    ...
                    if(schedules_dates.includes(moment(date).locale('en').format('DD'))) {
                        ...
                        if(ad_key == 0 && Object.keys($this.calculate_months).length != 1)
                        {
                            ...
                            if(moment($this.local_start_date, "YYYY-MM-DD").locale('en').format('DD') <= moment(date).locale('en').format('DD'))
                            {
                        ...
                        else if(ad_key == ($this.all_dates.length -1) && Object.keys($this.calculate_months).length != 1)
                        {
                            ...
                            if(moment($this.local_end_date, "YYYY-MM-DD").locale('en').format('DD') >= moment(date).locale('en').format('DD'))
                            {
                                $this.selected_dates[ad_key].push(moment(date).format('DD'));
                                // live-seat availability check
                                $this.addAvailableDates(ad_key, moment(date, 'YYYY-MM-DD').format('YYYY-MM-DD'), $this, true);
                        ...
                        if(Object.keys($this.calculate_months).length == 1)
                        {
                            ...
                            if(moment($this.local_start_date, "YYYY-MM-DD").locale('en').format('DD') <= moment(date).locale('en').format('DD') 
                                && moment($this.local_end_date, "YYYY-MM-DD").locale('en').format('DD') >= moment(date).locale('en').format('DD'))
                            {
                                $this.selected_dates[ad_key].push(moment(date).format('DD'));
                                // live-seat availability check
                                $this.addAvailableDates(ad_key, moment(date, 'YYYY-MM-DD').format('YYYY-MM-DD'), $this, true);
        ...
        generateDaily() {
            ...
            this.all_dates.forEach(function(ad_value, ad_key) {
                ...
                ad_value.forEach(function(v, k) {
                    ...
                    {
                        // selected date must be grather than start_date of event
                        if(moment($this.local_start_date,"YYYY-MM-DD").locale('en').format('DD') <= moment(v).locale('en').format('DD'))
                            all_dates_number[k] = moment(v).format('DD');
                    }
                    else if(ad_key == ($this.all_dates.length -1) &&  Object.keys($this.calculate_months).length != 1)
                    {
                        // selected date must be less than end_date of event
                        if(moment($this.local_end_date, "YYYY-MM-DD").locale('en').format('DD') >= moment(v).locale('en').format('DD'))
                            all_dates_number[k] = moment(v).format('DD');
                    }
                    else if(Object.keys($this.calculate_months).length == 1)
                    {
                        if(moment($this.local_start_date, "YYYY-MM-DD").locale('en').format('DD') <= moment(v).locale('en').format('DD') 
                                && moment($this.local_end_date, "YYYY-MM-DD").locale('en').format('DD') >= moment(v).locale('en').format('DD'))
                        {
                            all_dates_number[k] = moment(v).format('DD');
                        }    
                    }
                ...
                ad_value.forEach(function(date, key) {
                    ...
                    if(schedules_dates.includes(String(date.locale('en').format('dddd')))) {
                        ...
                        if(ad_key == 0 && Object.keys($this.calculate_months).length != 1)
                        {
                            // selected date must be grather than start_date of event
                            if(moment($this.local_start_date,  "YYYY-MM-DD").locale('en').format('DD') <= moment(date).locale('en').format('DD'))
                                $this.selected_dates[ad_key].push(Number(date.format('DD')));
                        }
                        else if(ad_key == ($this.all_dates.length -1) && Object.keys($this.calculate_months).length != 1)
                        {
                            // selected date must be less than end_date of event
                        ...
                        $this.selected_dates[ad_key].forEach(function(ad_v, ad_k) {
                            $this.available_dates[ad_key].push($this.checkSeatAvailability(moment($this.dateToFullDate(ad_v, $this.schedules[ad_key].from_date),'dddd LL').format('YYYY-MM-DD'), $this));
                        });
        ...
        convert_to_local_tz(){
            ...
            this.schedules.forEach(function(value, key) {
                ...
                this.local_from_date[key] = this.convert_date_to_local(this.schedules[key].from_date);
                this.local_to_date[key]   = this.convert_date_to_local(this.schedules[key].to_date);
                this.local_from_time[key] = this.convert_time_to_local(this.schedules[key].from_date, this.schedules[key].from_time, "HH:mm:ss");
                this.local_to_time[key]   = this.convert_time_to_local(this.schedules[key].from_date, this.schedules[key].to_time, "HH:mm:ss");
        ...
        weekly(){
            ...
            this.selected_dates.forEach(function(value, key){
                ...
                value.forEach(function(val1, key1) {
                    ...
                    if(!$this.week_numbers[key].includes(moment($this.dateToFullDate(val1, $this.schedules[key].from_date),'dddd LL').isoWeek())) {
                        $this.week_numbers[key].push(moment($this.dateToFullDate(val1, $this.schedules[key].from_date),'dddd LL').isoWeek());
                    }
                });
            });
        ...
        checkSeatAvailability(schedule_date, $this) {
            ...
            if(moment(schedule_date).add(1, 'day').format('YYYY-MM-DD') < moment().format('YYYY-MM-DD')) {
                return true;
            }
